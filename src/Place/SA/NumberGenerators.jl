# AbstractNumberGenerator interface
const _AMV_API = (:getlinear, :getoffset)

"""
    AbstractNumberGenerator{D}
...
"""
abstract type AbstractNumberGenerator{D} end

@doc """
    getlinear(a::AbstractNumberGenerator{D}, ub::Int)

Return a "random" number uniformly distributed from 1 to `ub`.
""" getlinear

@doc """
    getoffset(a::AbstractNumberGenerator{D}, limit::Int)
...
"""
struct RandomGenerator{D} <:AbstractNumberGenerator{D} end

# Use rand() on the range 1:ub.
getlinear(::RandomGenerator, ub::Int) = rand(1:ub)

# Use rand() for each component of the returned vector
@generated function MapperCore.getoffset(::RandomGenerator{D}, limit, ub) where D
...
"""
mutable struct SubRandomGenerator{D,V} <: AbstractNumberGenerator{D}
