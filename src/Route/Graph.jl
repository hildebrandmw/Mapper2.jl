...
function record!(new_dict::Dict, old_dict::Dict, offset::Integer, extension)
    ...
    for (path,graph_index) in old_dict
        ...
        new_path  = unshift(path, extension)
...
function add_subgraphs!(top::RoutingGraph, prefixes, subgraphs::Vector{T}) where T <: RoutingGraph
...
# NOTE: This code could be a lot faster and type stable - but works well enough
