...
#    metadata = Dict("capacity" => ASAP3_LINK_CAPACITY)
...
#	connect_ports!(comp, "processor.memory_out", "memory_out", metadata)
#	connect_ports!(comp, "memory_in", "processor.memory_in",  metadata)
#	# connect xbar i/o with processor i/o
#	for i = 0:num_links-1
#		connect_ports!(comp, string("processor.cs_out[",i,"]"), string("xbar[",i,"].in[0]"),metadata)
#		connect_ports!(comp, string("xbar[",i,"].out[0]"), string("processor.cs_in[",i,"]"),metadata)
#	end
...
#		count += 1
#		for i = 0:num_links-1
#			connect_ports!(comp, string("xbar[",i,"].out[",count,"]"), string(dir,"_out[",i,"]"),metadata)
#			connect_ports!(comp, string(dir,"_in[",i,"]"), string("xbar[",i,"].in[",count,"]"),metadata)
#		end
#	end
#	return comp
#
...
#    add_port(component, "cs_in", "input", 2, metadata = Dict("capacity" => 8))
#	# Add the processor cs output port
#    add_port(component, "cs_out", "output", 2, metadata = Dict("capacity" => 2))
